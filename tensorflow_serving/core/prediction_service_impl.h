...
class PredictionServiceImpl final : public PredictionService::Service {
 public:
  ...
  explicit PredictionServiceImpl(const Options& options)
      : core_(options.server_core),
        ...
        predict_platform_(options.platform) {
        if (options.platform == kPytorchModelPlatform){
          torch_predictor_.reset(new PytorchPredictor(options.use_saved_model));
        }else {
          predictor_.reset(new TensorflowPredictor(options.use_saved_model));
        }
  };
 ...
 private:
  ...
  std::unique_ptr<PytorchPredictor> torch_predictor_;
  string predict_platform_;
